...
class Crowd(data.Dataset):
    def __init__(self, root_path,transform=None, train=False, patch=False, flip=False, crop_size=256,
                 downsample_ratio=8,
                 method='train'):
...
def loading_dataPRCV(data_root):
    # the pre-proccssing transform
    transform = standard_transforms.Compose([
        standard_transforms.ToTensor(), 
        standard_transforms.Normalize(mean=[0.485, 0.456, 0.406],
    ...
    # create the training dataset
    train_set =Crowd(data_root, train=True, transform=transform, patch=True, flip=True)
    # create the validation dataset
    val_set = Crowd(data_root, train=False, transform=transform)
